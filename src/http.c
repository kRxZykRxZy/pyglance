...
static void login_api(int fd,const char *req) {
    ...
    if(body) body+=4;
    if(body){
        sscanf(strstr(body,"username=")?strstr(body,"username=")+9:"","%127[^&]",user);
        sscanf(strstr(body,"password=")?strstr(body,"password=")+9:"","%255[^&]",pass);
    }
...
static void api(int fd,const char *req,const char *path) {
    ...
    if(!strncmp(path,"/api/signal?",12)){
        int pid=0,sig=0; sscanf(path,"/api/signal?pid=%d&sig=%d",&pid,&sig);
        response(fd,process_signal(pid,sig)==0?"200 OK":"400 Bad Request","application/json",process_signal(pid,sig)==0?"{\"ok\":true}":"{\"ok\":false}",NULL); return;
    }
...
void handle_connection(int fd) {
    ...
    if(!strcmp(path,"/")||!strcmp(path,"/login")){login_page(fd);}
    else if(!strcmp(method,"POST")&&!strcmp(path,"/api/login")){login_api(fd,req);}
