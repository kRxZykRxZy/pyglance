...
static void login_api(int fd,const char *req){
    ...
    if(!strcmp(user,PI_MONITOR_USER)&&!strcmp(pass,PI_MONITOR_PASSWORD))response(fd,"200 OK","application/json","{\"ok\":true}","Set-Cookie: " PI_MONITOR_COOKIE "; Path=/; HttpOnly; SameSite=Strict\r\n");
...
static void api(int fd,const char *req,const char *path){
    ...
    if(!strncmp(path,"/api/signal?",12)){int pid=0,sig=0;sscanf(path,"/api/signal?pid=%d&sig=%d",&pid,&sig);int ok=process_signal(pid,sig)==0;response(fd,ok?"200 OK":"400 Bad Request","application/json",ok?"{\"ok\":true}":"{\"ok\":false}",NULL);return;}
    response(fd,"404 Not Found","application/json","{\"error\":\"not found\"}",NULL);
}

void handle_connection(int fd){
    char req[16384]={0},method[16]={0},path[512]={0};int n=recv(fd,req,sizeof(req)-1,0);if(n<=0){close(fd);return;}sscanf(req,"%15s %511s",method,path);
    if(!strcmp(method,"POST")&&!strcmp(path,"/api/login"))login_api(fd,req);
    else if(!strcmp(path,"/")||!strcmp(path,"/login")){file_response(fd,PI_MONITOR_WEB_ROOT "/login.html","text/html; charset=utf-8");}
    ...
    else if(!strcmp(path,"/dashboard")){if(logged_in(req))file_response(fd,PI_MONITOR_WEB_ROOT "/dashboard.html","text/html; charset=utf-8");else file_response(fd,PI_MONITOR_WEB_ROOT "/login.html","text/html; charset=utf-8");}
