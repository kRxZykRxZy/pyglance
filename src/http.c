...
static void login_api(int fd,const char *req){
    ...
    if(body) body+=4;
    if(body){
        const char *u=strstr(body,"username="); const char *p=strstr(body,"password=");
        if(u) sscanf(u+9,"%127[^&]",user);
        if(p) sscanf(p+9,"%255[^&]",pass);
    }
...
static void api(int fd,const char *req,const char *path){
    ...
    if(!strncmp(path,"/api/signal?",12)){
        ...
        int ok=process_signal(pid,sig)==0;
        response(fd,ok?"200 OK":"400 Bad Request","application/json",ok?"{\"ok\":true}":"{\"ok\":false}",NULL);
        return;
    }
